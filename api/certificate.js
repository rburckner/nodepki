...
certificate.request = function (req, res) { // Validate user input
    ...
    fs.writeFile(tempdir + 'request.csr', csr, function (err) {
      if (err === null) { // OpenSSL command template
        ...
        exec(signcommand, {
          ...
          if (error === null) { // Re-index DB
            certdb.reindex().then(function () { // Read generated certificate file
              fs.readFile(tempdir + 'cert.pem', 'utf8', function (err, certdata) {
                if (err === null) { // Send certificate to client
                  respond({
                    success: true,
                    cert: certdata
                  }, res);
  ...
  }).then(function () { // Clean up... Remove temporary files
...
certificates.list = function (req, res) { // Validate user input
  ...
  switch (data.state) {
    case 'all': filter = '';
      break;
    case 'revoked': filter = 'R';
      break;
    case 'valid': filter = 'V';
      break;
    case 'expired': filter = 'E';
      break;
    default:
      // Filter is invalid
      respond({
...
certificate.get = function (req, res) { // Validate user input
  ...
  } else { // Certificate file not found.
